...
devicesdef = [{'index':1, 'name':'Temperature', 'description':'Capteur de temperature', 'type':80, 'subType':5 },
              ...
              {'index':6, 'name':'Niveau PH', 'description':'Vieau PH', 'type':249, 'subType':19, 'image':'xfr_chlore.zip'}]
class BasePlugin:
    ...
    def handleConnection(self, Data = None):
        ...
        if self.lastDateTime != jsonData['DateTime']:
            ...
            PHText = str(jsonData['PH']['Message'])
            self.createAndAddToDevice(6, PHText, 243, 19)
